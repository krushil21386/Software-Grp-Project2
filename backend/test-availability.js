...
async function testAvailability() {
  try {
    ...
    const getRes = await axios.get('http://localhost:5001/api/doctors/schedule-test', {
    ...
    const updateRes = await axios.put('http://localhost:5001/api/doctors/schedule-test', 
    ...
    // 4. Verify update
    const verifyRes = await axios.get('http://localhost:5001/api/doctors/schedule-test', {
      headers: { Authorization: `Bearer ${token}` }
    });
